    ...
    <form action="{{ route('users.update', $user->id) }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="username" id="userUsername" value="{{ old('username', $user->username) }}"
            ...
            <div>
                ...
                <input type="text" name="name" id="userFullName" value="{{ old('name', $user->name) }}"
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    required>
            </div>
        </div>
        ...
        <div>
            ...
            <input type="email" name="email" id="userEmail" value="{{ old('email', $user->email) }}"
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div class="space-y-2">
                    ...
                    <div class="border-t border-gray-200 pt-2">
                        <div class="grid grid-cols-1 gap-2" id="userBranchCheckboxes">
                            ...
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" value="{{ $plant->name }}" name="userBranches[]"
                                    class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                                    @if(in_array($plant->name, old('userBranches', []))) checked @endif>
                                <span class="text-sm">{{ $plant->name }}</span>
                            </label>
                            @endforeach
                        </div>
                    </div>
